...
- 런타임 저장 스키마 초안:
  ...
  - 보유 스킬/쿨다운 상태
  - 보유 구조물
  - 보드 위 몬스터 상태(현재 체력/현재 행동/행동 준비 카운트)
  - 경제 값(골드)
...
- 런 보드 UI 요구사항 초안:
  - 몬스터 카드별 태그, 체력, 현재 행동, 행동 효과 요약, 행동 준비 카운트 표시
  - 모험가 배치 가능/불가 상태 가시화
...
2. 모험가 처리 루프(모험가 수만큼 반복):
   ...
   - 굴림값을 스킬/칙령으로 조정합니다.
   - 조정이 끝난 모험가를 `Enemy`에 드래그해 공격으로 소비합니다.
3. 종료 정산(생존 몬스터만 대상):
...
- `1/2/3/4` 단축키는 좌측 모험가 슬롯(위->아래)과 고정 매핑합니다.
...
- 현재 처리 중 모험가는 카드 외곽선 강조 + 헤더 배지로 명확히 표시합니다.
...
- 초기 상태
  ...
  - 구조물: 이번 턴 최저 눈 +1
- 보드 상황
  - `오크 부대`(침략): 체력 20, 현재 행동 `강습`(준비 1, 안정도 -2)
  - `메뚜기 떼`(기근): 체력 17, 현재 행동 `황폐화`(준비 2, 기근 관련 페널티)
- 배치/굴림 결과
  - 오크 부대에 모험가 배치 -> 공격 합계 9
  - 메뚜기 떼에 모험가 배치 -> 공격 합계 6
- 판정
  - 오크 부대 체력 11 남음, 메뚜기 떼 체력 11 남음
  - `P5`에서 오크 부대 행동이 발동(준비 1 -> 0)하고, 다음 행동을 다시 선택
  - 다음 턴에 잔여 체력(오크 11, 메뚜기 11)을 이어서 공략
