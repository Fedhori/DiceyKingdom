...
중세 다크 판타지에서, 왕은 **3개의 전장**에 주사위 병력을 분배하고 적의 의도를 읽어 스킬로 전황을 뒤집는다. 단, **전투 패배는 즉시 게임오버**이며 **후퇴는 Honor가 허락할 때만 가능**하다.
...
- 전투 종료 조건: 한쪽 Health이 `<= 0`
- 게임오버 조건: **플레이어가 전투에서 패배(플레이어 Health `<= 0`)**
- Honor는 게임오버 조건이 아니다.
  - Honor는 **후퇴 자원**이다.
  - Honor가 `<= 0`이면 **후퇴 불가**
...
1) **분배 퍼즐**: 적 의도(Opponent Intent)를 보고, 내 병력을 3개 전장에 어떻게 분배할지 고민
2) **편성 고민**: Supply Limit 안에서 어떤 Squad/Support를 들고 갈지 결정
3) **리스크 관리**: 배치 페이즈마다 “이번 전투를 계속할지 / 후퇴할지”를 Honor를 기준으로 판단
...
1) Reset
   - 전장에 배치되어 있던 모든 병력은 ActionHolder로 돌아온다.

2) Opponent Deploy
   - 적이 Opponent Intent대로 병력을 전장에 배치한다.

3) Player Deploy
   - 플레이어가 ActionHolder의 병력을 전장에 배치한다.
   - 이 페이즈에서 Retreat 가능(보스전 제외).
     - Retreat 가능 조건: `Honor > 0`
     - Retreat 시: 전투 즉시 종료, 보상 없음, `Honor -= 1`, 최소 0으로 clamp
...
6) ClashResolve
   ...
   - **각 전장 판정 직후 양측 Health을 즉시 체크**하며, `<= 0`이면 그 즉시 전투가 종료될 수 있다.
...
- 전장 Total Attack는 다음의 합으로 계산한다.
  1) 전장 내 모든 Action의 **최종 Attack Result 합**
...
- 재배치(Redeploy)
  ...
  - 아군 Action 1개를 다른 전장으로 이동(슬롯 제한 준수)

- 유인책(Decoy)
  - Focus 2 / Cooldown 2 / Deploy
  - 적 Action 1개를 다른 전장으로 이동(슬롯 제한 준수)
...
- ClashResolve: 전장 0→1→2 순서대로 판정, 매 판정 후 Health 즉시 체크
- Retreat:
  - Player Deploy에서만
  - Honor > 0일 때만 가능
  - Retreat 시 전투 종료, 보상 없음, Honor -1, clamp 0
- Focus:
  - 최대 5
  - 전투 시작 시 최대 충전
  - 턴 종료 시 +2 회복
