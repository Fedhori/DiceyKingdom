...
3. 모험가 순차 처리 루프
   - 모험가 1명 배치 -> 해당 모험가 굴림 -> 보정/공격 확정
   - 남은 모험가가 있으면 반복
4. 턴 종료 정산(생존 몬스터 행동 카운트 감소, 행동 발동 효과 적용, 안정도 증감)
5. 다음 턴 반복
...
- 런 보드 UI 요구사항 초안:
  - 몬스터 카드별 태그, 체력, 현재 행동, 행동 준비 카운트 표시
  - 모험가 배치 가능/불가 상태 가시화
...
- 몬스터는 `태그`, `체력(Health)`, `행동(Action)`을 가집니다.
...
- v0 처리 규칙:
  ...
  - `P4`에서 `current_health <= 0`이면 즉시 처치 후 보드에서 제거합니다.
...
- `P1 BoardUpdate`
  ...
  - `P2 Assignment`
    - 현재 처리할 모험가를 `Enemy` 단위로 배치
    - `Window`: 스킬/칙령 사용 가능
    - `Commit`: 현재 모험가-몬스터 귀속 확정
  - `P3 Roll`
    - 현재 모험가 주사위를 굴려 결과 생성
    - `Commit`: 현재 모험가 원시 굴림값 확정
  - `P4 Adjustment`
    - 현재 모험가 굴림값/몬스터 상태를 보정하는 효과 처리
    - `Window`: 스킬/칙령 사용 가능
    - 현재 모험가 공격으로 몬스터 체력을 감소
    - 몬스터 체력 0 이하 시 즉시 `처치`
    - `Commit`: 현재 모험가 최종 적용값 확정
    - 남은 모험가가 있으면 `P2`로 돌아가 다음 모험가를 처리
  - `P5 Settlement`
    - 내부 순서:
...
- v0 기본 스킬 3종의 사용 윈도우는 아래로 고정합니다.
  - 리롤(`reroll_adventurer_dice`): `P4` 전용
  - 눈 +1(`die_face_delta`): `P4` 전용
  - 적 피해 2(`enemy_health_delta`): `P2`, `P4`
- 순차 처리에서는 "현재 처리 중인 모험가"를 기준으로만 대상 지정이 가능합니다.
- 스킬 입력 방식은 `스킬 슬롯 클릭` 또는 `숫자 단축키(1,2,3,4)`로 스킬 선택 후, 유효 타깃 클릭으로 고정합니다.
...
- `P2 Assignment`에서는 현재 처리 중인 모험가 1명을 `Enemy`에 배치합니다.
- v0에서는 `롤 후 할당` 방식을 사용하지 않고, `사전 할당 -> 롤 -> 조정` 순서를 유지합니다.
- 턴 내 모험가 처리 순서는 "하나 배치 -> 롤 -> 조정 -> 확정"을 반복하는 순차 처리입니다.
- 턴 내 모험가 처리 순서는 `플레이어 자유 선택`으로 고정합니다(슬롯 고정 순서 아님).
- 현재 모험가를 확정한 뒤 `Undo`는 허용하지 않습니다.
- `P4`에서 체력 0 이하가 된 `Enemy`는 즉시 보드에서 제거되며, 이후 페이즈에서 배치 대상으로 취급하지 않습니다.
- `P2`에서 배치된 모험가는 `P4`에서 같은 `Enemy`를 공격하는 데만 사용할 수 있습니다.
- `P4` 공격 순서는 각 `Enemy.assigned_adventurer_ids`의 순서를 따릅니다.
  - 즉, 같은 `Enemy`에 대해 먼저 드래그해 배치한 모험가가 먼저 공격합니다.
...
- `P4` 종료 시점에 미배치 모험가가 있으면 경고 모달을 표시합니다.
...
- 레이아웃은 좌-우 대치 구도:
  - 좌측: 모험가
  - 우측: Enemy
- 할당 상태는 `상시 연결 라인`으로 표시합니다.
- 입력 방식은 `드래그 전용`으로 제한합니다(클릭 할당 미지원).
- 드래그 중에는 연결 라인이 대상까지 늘어나는 방식으로 표시합니다.
- 모험가를 드래그해 `빈 공간`에 드롭하면 해당 모험가의 배치를 해제합니다.
- 모바일 UI는 v0 범위에서 고려하지 않습니다(데스크톱 우선).
- 드래그 드롭이 완료되면 현재 모험가는 즉시 `P3 Roll`로 진입합니다.
- `P4` 확정은 단축키 `Space`와 UI `확정` 버튼을 모두 지원합니다.
- UI `확정` 버튼은 `P4`에서만 표시하며, 위치는 하단 중앙(Footer Primary CTA)으로 고정합니다.
