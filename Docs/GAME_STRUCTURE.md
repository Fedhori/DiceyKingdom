...
- `RunScene`에서 단일 런이 진행되며, 턴 단위로 몬스터 생성/공격/정산을 반복
- 런 종료 조건: 안정도 0 이하

## 핵심 루프

1. 턴 시작
2. 몬스터 보드 갱신(신규 몬스터 추가, 지속 효과 갱신)
3. 모험가 배치(몬스터 단위 할당)
4. 주사위 굴림
...
1. 턴 시작: 신규 몬스터가 규칙에 따라 보드에 추가됩니다.
...
- 사이클 정의
  - `P0 TurnStart`
    ...
    - `Commit`: 리필/쿨다운 결과 확정
  - `P1 BoardUpdate`
    - 신규 몬스터 생성, 보드 상태 갱신(턴 시작형 지속효과 포함)
