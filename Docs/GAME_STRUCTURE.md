...
- 기존 표현은 점진적으로 `전령(Advisor)`, `구조물(Structure)`, `칙령(Decree)` 기준으로 통일합니다.
...
3. 주사위 배치(상황 단위 할당)
4. 주사위 굴림
5. 전령/칙령 보정 후 슬롯 해소(상황 성공 시 보상 즉시 지급)
...
- 일부 전령은 상황당 사용 횟수 제한을 가집니다.
- 쿨다운이 0이 되면 해당 턴에 즉시 사용 가능합니다.
- 무쿨다운 전령도 허용하며, 이 경우 골드 등 대체 비용을 요구할 수 있습니다.
...
- 용어 정의
  - `Phase`: 턴을 구성하는 고정 단계
  - `Window`: 플레이어가 전령/칙령 등 능동 효과를 사용할 수 있는 구간
  - `Commit`: 해당 단계 결과가 확정되어 되돌릴 수 없는 지점
- 사이클 정의
  - `P0 TurnStart`
    - 주사위 리필, 전령 쿨다운 감소, 턴 시작 트리거 처리
    - `Commit`: 리필/쿨다운 결과 확정
  - `P1 BoardUpdate`
...
- 플레이어 보유 전령의 쿨다운은 턴 시작 시 감소합니다.
- 쿨다운이 0이 된 전령은 같은 턴에 즉시 사용 가능합니다.

### 전령/칙령 사용 규칙(확정)

- 턴당 전령 사용 횟수 제한은 없습니다.
- 턴당 소모품 사용 횟수 제한은 없습니다.
- 효과 비용은 대부분 무료를 기본값으로 보되, 일부 전령/칙령은 골드 비용을 가질 수 있습니다.
- 사용 윈도우는 `P2 Assignment`, `P4 Adjustment`로 제한합니다.
- `P3 Roll` 및 `P5 Settlement` 이후에는 전령/칙령을 사용할 수 없습니다.
- 비용이 있는 효과는 항상 사용 전에 비용을 지불합니다.
- 골드가 부족하면 해당 효과는 사용할 수 없습니다.
- 사용 가능/불가 상태는 UI에서 즉시 식별 가능해야 합니다.
- 소모품은 턴당 제한이 아니라 보유량 한도로 관리합니다.
- 무쿨다운 비용형 전령의 글로벌 하드 제한은 두지 않고, 개별 콘텐츠 설계로 위험도를 제어합니다.
...
- 예외적으로, 명시된 전령/칙령 효과가 있을 때만 `Situation` 간 이동/교환을 허용합니다.
- `P4`에서 상황이 해결되어 획득한 모든 것(골드/칙령/전령 포함)은 같은 `P4`에서 즉시 사용 가능합니다.
...
- 턴 파워 예산 `P_turn`은 아래 합으로 관리합니다.
  ...
  - `P_skill_free`: 이번 턴 사용 가능한 무료 Advisor 파워
  - `P_skill_paid`: 골드 비용을 지불해 전환 가능한 Advisor 파워
...
- 목표 상태:
  ...
  - 고위험 턴에서는 비용형 Advisor/Decree로 `P_skill_paid`, `P_item`을 동원해 위기 탈출 가능해야 함
