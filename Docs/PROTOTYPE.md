...
- 산출물
  - `Game.Domain.Duel`
    ...
    - `ActionInstance`
  - 상태 최소 필드
    - Health / Focus / Turn / Cooldown / ActionHolder / Clashes / OpponentIntent 참조
...
- [x] `T1-02` 파일/네임스페이스 스켈레톤 생성
  ...
  - 네임스페이스: `Game.Domain.Duel`
  - 클래스 3종 생성: `DuelState`, `ClashState`, `ActionInstance`
...
- [x] `T1-04` `ClashState` 최소 구조 구현
  - `playerActions`, `opponentActions`, `totalAttackBonusPlayer`, `totalAttackBonusOpponent`, `slotLimit` 필드를 구현한다.
  - 컬렉션 null 방지 초기화를 적용한다.

- [x] `T1-05` `ActionInstance` 굴림 추적 필드 구현
  - `actionDefId`, `Attack`, `baseRoll`, `attackResult`, `modifiers`, `tags` 필드를 구현한다.
...
- [x] `T1-07` EditMode 테스트 추가
  - `DuelState` 생성 시 필수 컬렉션 null 아님
  - `clashes.Count == 3`
  - `ActionInstance`의 `base/mod/final` 필드가 생성 직후 접근 가능
...
### 작업 2: Phase 오케스트레이션 고정(DuelPhaseRunner)

**목표:** 페이즈 진행 순서와 전환 규칙을 Application 계층에서 고정한다.

- 산출물
  - `Game.Application.Duel.DuelPhaseRunner`
  - `Phase enum`
    - Reset / OpponentSetup / PlayerSetup / Roll / Skill / ClashResolve
  - 전환 API
    - `StartDuel`
    - `AdvanceToNextPhase`
    - `TryRetreat` (PlayerSetup + Honor 조건 강제)

- 수동 테스트
  - 페이즈 순서가 확정 규칙대로만 진행되는가
  - 허용되지 않은 시점의 Retreat 요청이 거부되는가
  - ClashResolve 이후 Turn End 내부 처리로 돌아가는가

- 자동 테스트
  - 페이즈 전이 테스트(정상/비정상 전이)
  - Retreat 조건 테스트(Honor > 0, PlayerSetup 한정)
...
- [x] `T2-01` 페이즈 타입 고정
  - `DuelPhase` enum을 고정한다.
  - 순서: `Reset -> OpponentSetup -> PlayerSetup -> Roll -> Skill -> ClashResolve`

- [x] `T2-02` 실패 사유 타입 고정
  - `DuelPhaseFailureReason` enum을 고정한다.
  - 최소 항목: `None`, `NotStarted`, `InvalidPhase`, `HonorInsufficient`, `AlreadyEnded`

- [x] `T2-03` Runner 스켈레톤 생성
  - 경로: `Assets/Scripts/Game/Application/Duel/DuelPhaseRunner.cs`
  - 네임스페이스: `Game.Application.Duel`
  - 순수 C# 클래스로 구현(MonoBehaviour 금지)
  - 상태는 `DuelState`를 직접 수정(in-place)한다.
...
- [x] `T2-05` StartDuel 구현
  - 시작 시 현재 페이즈를 `Reset`로 둔다.
  - 이미 종료 상태면 실패 처리한다(`AlreadyEnded`).

- [x] `T2-06` AdvanceToNextPhase 구현
  - 확정 순서대로만 전이한다.
  - `ClashResolve` 다음에는 내부 Turn End 처리 후 `Reset`로 복귀한다.
  - Turn End 최소 처리: `turnIndex` 증가.

- [x] `T2-07` TryRetreat 구현
  - `PlayerSetup`에서만 허용한다.
  - `honor > 0`일 때만 성공한다.
  - 성공 시 `honor -= 1` (최소 0), `isDuelEnded = true`.
...
- [x] `T2-09` EditMode 테스트 추가
  - 정상 페이즈 전이 테스트
  - `ClashResolve -> Reset` 복귀 + `turnIndex` 증가 테스트
  - Retreat 성공/실패(페이즈 오류, honor 부족) 테스트
...
- [x] `T3-01` 계산 타입/진입점 고정
  - `DuelOutcome` enum 추가
  - `DuelSimulator` 정적 진입점(`RollAction`, `ApplyRollFinalization`, `ComputeTotalAttack`, `ComputeOutcome`, `ClashResolveClashesInOrder`) 고정
...
- 산출물
  - `EffectClashResolver` + `OpCode 핸들러(Dictionary<OpCode, IOpHandler>)`
  - P0 opcode 최소:
...
- [x] `T6-01` `DuelDebugPanel` 스켈레톤 생성
  - 경로: `Assets/Scripts/Game/Presentation/Debug/DuelDebugPanel.cs`
  - 네임스페이스: `Game.Presentation.Debug`
  - TMP_Text/Button 참조 필드와 콜백 시그니처를 먼저 고정한다.

- [x] `T6-02` 패널 상태 컨텍스트 고정
  - 패널 내부에 `DuelState`, `DuelPhaseRunner`, `selectedActionId`, `selectedClashIndex`를 유지한다.
...
- [x] `T6-06` 페이즈별 버튼 활성/비활성 규칙 적용
  ...
  - `Roll`: `Roll` 페이즈에서만 활성
  - `ClashResolve`: `ClashResolve` 페이즈에서만 활성
...
- [x] `T6-12` 상태 포맷터 분리 + EditMode 테스트
