...
- [ ] 1.0 상점 핀 상품 선택 상태 관리 및 이벤트 발행 구조 추가
  - [ ] 1.1 선택 상태를 보관할 필드/프로퍼티 정의(선택 PinInstance 또는 pinId, null 시 해제) 및 이벤트(Action 등) 발행 메커니즘 추가
  - [ ] 1.2 ShopItemView 클릭 시 구매 직행 대신 “선택 Setter → 이벤트 Invoke”로 흐름 전환, 다른 아이템 선택 시 이전 선택 해제 처리
  - [ ] 1.3 상점 열림/닫힘 시 선택 상태 초기화(이벤트로 null 전달)하여 배경/하이라이트가 남지 않도록 정리
  - [ ] 1.4 메인 스토어 한정 동작 유지(다른 상점 컨텍스트와 혼동되지 않도록 가드)
