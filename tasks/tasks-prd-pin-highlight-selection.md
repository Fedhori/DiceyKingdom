...
- `Assets/Scripts/Pin/PinDragManager.cs` - 핀 드래그 진행/스왑/하이라이트 상태 관리.
...
- [x] 2.0 선택된 핀 상품 UI 배경/하이라이트 표시 연동 (`Colors.HighlightColor`)
...
- [x] 3.0 기본(pin.basic) 핀 하이라이트 토글 및 클릭 시 구매/배치 처리 (`PinManager.TryReplace`)
  ...
  - [x] 3.3 구매 성공 시 선택 상태 해제 및 하이라이트/배경 초기화, 실패 시 통화/Replace 실패 대응(환불/로그) 후 상태 일관성 유지
- [ ] 4.0 핀 드래그 입력을 PinController로 이동 (PinDragManager 입력 제거)
  - [x] 4.1 PinController에 드래그 핸들러 추가(IBeginDragHandler/IDragHandler/IEndDragHandler)로 드래그 입력 전달
  - [x] 4.2 PinDragManager에서 Update/Mouse 로직 제거, Begin/Update/End/Cancel API로 교체
  - [ ] 4.3 드래그 중 상태/하이라이트/스왑 및 취소 시 원복이 정상 동작하는지 정리
