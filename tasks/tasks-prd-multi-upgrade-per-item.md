...
- `Assets/Scripts/GameConfig.cs` (또는 기존 설정 파일) - 최대 강화 슬롯 수 상수 추가.
...
- [ ] 1.0 다중 강화 데이터 모델 도입 및 최대 슬롯 상수 추가
  - [ ] 1.1 `GameConfig`에 최대 강화 슬롯 수 상수 추가 및 접근자 정리
  - [ ] 1.2 `ItemInstance`에 강화 리스트 보관 및 변경 알림(이벤트/콜백) 추가
  - [ ] 1.3 강화 리스트를 기준으로 스탯 합산/복제 로직이 정상 작동하도록 업데이트
- [ ] 2.0 강화 적용/제거/교체 로직을 다중 강화 기준으로 리팩터링
  - [ ] 2.1 `UpgradeManager`의 적용/해제 API를 리스트 기반으로 변경하고 중복 허용 처리
...
- [ ] 3.0 아이템 슬롯에 강화 아이콘 리스트 표시 UI 추가
  ...
  - [ ] 3.2 아이템 변경/강화 변경 시 아이콘 리스트를 갱신하는 메서드 추가
  - [ ] 3.3 희귀도 배경색/아이콘 스프라이트 매핑 적용
