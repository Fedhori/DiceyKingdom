...
- `Assets/Scripts/Tooltip/ItemTooltipUtil.cs` - 아이템 툴팁 버튼 설정 전달.
...
- [x] 5.0 교체 모달/버튼 로컬라이즈 키 추가 및 문구 반영
  - [x] 5.1 `tooltip.upgrade.replace.label` 키 추가 및 버튼 라벨 연결
  - [x] 5.2 `modal.upgradeReplace.title/body` 키 추가 및 확인 모달 호출 연결
- [x] 6.0 아이템 판매 시 모든 강화가 강화 가방으로 이동되도록 수정
  - [x] 6.1 아이템 판매 시 강화 리스트를 회수해 강화 가방에 추가하는 로직 수정
  - [x] 6.2 판매 후 아이템의 강화 리스트를 비우고 UI 아이콘을 갱신
