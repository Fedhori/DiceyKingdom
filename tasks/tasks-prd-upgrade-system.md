...
- [ ] 1.0 강화 데이터 파이프라인 구축
  - [ ] 1.1 `Upgrades.json` 스키마 정의 및 샘플 강화(힘의 돌/바람의 돌) 데이터 추가
  - [ ] 1.2 `UpgradeDto`/조건/효과 구조 설계 및 JSON 파싱 로직 구현
...
- [ ] 2.0 상점에 강화 상품 혼합 등장 및 전용 UI 연결
  ...
  - [ ] 2.2 `ShopItemFactory`에 강화 상품 생성 로직 추가
...
- [ ] 3.0 강화 적용/교체 및 아이템 스탯 반영 처리
  - [ ] 3.1 `StatLayer.Upgrade` 추가 및 아이템 스탯 적용 경로 정리
  - [ ] 3.2 아이템에 강화 1개 보관 필드 추가(교체 시 기존 제거)
