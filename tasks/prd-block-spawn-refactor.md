...
## 1. 소개/개요
현재 블럭 스폰 로직은 “기본(normal)”과 “큰(big)” 두 패턴만 지원한다. 이를 가중치/비용/수치 속성(size/speed/health)을 가진 확장 가능한 패턴 시스템으로 대체하여, 런타임 중 패턴 풀과 패턴 속성이 가변적으로 변할 수 있는 구조로 개선한다.

## 2. 목표
- 패턴 기반 스폰 구조로 전면 전환한다.
- 패턴은 가중치(weight), 비용(cost), 수치 속성(size/speed/health)을 가진다.
- 가중치 랜덤으로 패턴을 선택한다.
...
2. `BlockPattern`은 최소한 `weight`, `cost`, `size`, `speed`, `health`를 가져야 한다.
