...
- [ ] 1.0 설계/데이터 계산 준비
  ...
  - [ ] 1.2 스폰 지점 프리팹 스펙 정의(스프라이트, 알파 페이드, Collider2D)
- [ ] 2.0 스폰 지점 표시/입력 처리
  - [ ] 2.1 스폰 지점 생성/표시 스크립트(지점 매니저) 추가: 라운드 대기 시 9개 지점 생성, 라운드 시작 시 제거/비활성화
  - [ ] 2.2 알파 페이드 점멸 효과 구현(코루틴/트윈 등) 및 프리팹 바인딩
  - [ ] 2.3 터치/클릭 입력 처리: 단일 입력으로 지점 선택 → 콜백으로 선택 좌표 전달
- [ ] 3.0 라운드 흐름 연동
  - [ ] 3.1 Flow/StageManager에 “스폰 지점 선택 대기” 상태 추가 또는 RoundStart 버튼 클릭 시 지점 표시 후 대기하도록 변경
