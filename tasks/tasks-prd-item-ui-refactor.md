...
- [ ] 3.0 Shop/Slot/Ghost 연결 코드 갱신 및 데이터 바인딩 정리
  ...
  - [x] 3.2 ShopManager의 선택/드래그 이벤트가 ProductController와 일관되게 연결되도록 갱신
  ...
  - [x] 3.7 슬롯/상점 드래그 시 유효 위치 프리뷰 렌더링 추가
  - [x] 3.8 유효한 슬롯 호버 시 드래그 고스트 숨김 처리
  - [x] 3.9 상점 드래그 이탈 시 빈 슬롯 하이라이트 복구
  - [x] 3.10 슬롯 드래그 중 원본 슬롯 아이콘 숨김
  - [x] 3.11 슬롯 드랍 영역용 RectTransform 분리 지원
  - [x] 3.12 상점 드래그 시 슬롯 하이라이트 토글 제거
