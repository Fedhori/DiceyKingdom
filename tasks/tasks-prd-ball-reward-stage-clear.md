...
- [ ] 1.0 RewardManager 보상 흐름 확장
  - [ ] 1.1 Reward 단계 진입 시 `currentBallRerollCost = baseBallRerollCost`, `ballRewardOverlay` 활성화, 이전 보상 인스턴스 정리.
  - [ ] 1.2 `BallRepository`에서 `isNotReward == false`인 볼 목록을 가져와 `ballRewardCount`개 보상 생성(고유 우선, 부족 시 중복 허용).
