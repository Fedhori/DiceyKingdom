...
- `Assets/Scripts/Shop/ShopItemData.cs` - 상점 아이템 데이터 구조; Ball도 유사 구조 필요 시 재사용/확장 검토.
- `Assets/Scripts/Shop/ShopItemView.cs` - Pin용 뷰; BallItemView 구조 참고용.
...
- [ ] 1.0 Ball 상점 슬롯 구성/데이터 로딩 추가
  - [ ] 1.1 `isNotSell == false` Ball 풀 구성 및 슬롯 수(기본 2, 변수화) 랜덤 노출 로직 추가
  - [ ] 1.2 재롤/회차마다 Ball 상품 재생성 및 중복 방지 처리
- [ ] 2.0 Ball 전용 뷰/툴팁 연결
